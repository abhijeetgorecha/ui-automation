...
public class KeyDrivenTestCasesSuiteRunnerCI extends KeyDrivenTestingSuiteBase {
	 ...
	 */
		...
		@Parameters({ "browser" })
			public void Test_13$Test3(@Optional("") final String browser){				
					
			final ExecutionEngine engine = new ExecutionEngine();
			final TestCase testCase = testCaseMap.get("Test3");
			...
			try {
				...
				reporterCache.get("Test3").fnWriteToHtmlOutput("Test3","Test case should gets pass","Test case Failed" , "Fail");
				}
				else{
					reporterCache.get("Test3").fnWriteToHtmlOutput("Test3","Test case should gets pass","Test case Passed" , "Pass");	
				}
				afterTest(testResultStatus,testCase.getTestCaseName());
